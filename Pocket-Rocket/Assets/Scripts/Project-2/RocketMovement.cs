...
public class RocketMovement : MonoBehaviour
{
    ...
    // Fuel System
    [SerializeField,Range(100f,500f)] private float fuelCapacity=100;
    [SerializeField, Range(100f, 500f)] public float currentFuelCapacity;
    [SerializeField, Range(100f, 500f)] private float refuelCapacity = 100f;
    [SerializeField] float consumptionRate = 1f;
    ...
    public void processThrust()
    {
        if (Input.GetKey(KeyCode.Space))
        {
            ...
            // Burns the Fuel
            currentFuelCapacity -= consumptionRate * Time.deltaTime;
        ...
        else if (Input.GetKey(KeyCode.X))
        {
            ...
            // Burns the Fuel by half
            currentFuelCapacity -= consumptionRate *.5f* Time.deltaTime;
            // Checks Whether there is already the same audio file running
            if (!ads.isPlaying)
            {
    ...
    public void Refuel()
    {
        currentFuelCapacity += refuelCapacity;
    }
    ...
    public void CheckFuel()
    {
        ...
        {
            ch.StartCrashSequence();
        }
    }

    void OnTriggerEnter(Collider other)
    {
        if(other.gameObject.tag.Equals("FuelObject"))
        {
            Debug.Log("Refueling");
